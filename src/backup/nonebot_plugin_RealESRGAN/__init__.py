...
try:
    ...
    api = 'https://elainafanboy-irony-real-esrgan.hf.space/api/predict/'
...
@real_esrgan.got("mode", prompt="请提供重建模式(二次元：anime，其它：base)，模式不绝对，可以任意选", parameterless=[HandleCancellation("已取消")])
async def real_esrgan_get_mode(event: MessageEvent, state: T_State):
    mode = str(state["mode"]).strip()
    if mode not in ["anime", "base"]:
        await real_esrgan.finish('"base" | "anime", 二选一，超分已取消')
...
async def real_esrgan_handle_img(event: MessageEvent, state: T_State):
    ...
    try:
        ...
        if result is None:
            await real_esrgan.finish(Message(f"[CQ:at,qq={state['id']}]这张图没能被正确解析，可能网络连接失败或者是由于免费额度耗尽，如果是后者建议联系管理员"))
