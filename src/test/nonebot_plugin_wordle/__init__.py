...
wordle = on_shell_command("wordle", parser=parser, block=True, priority=10)
...
def shortcut(cmd: str, argv: List[str] = [], **kwargs):
    command = on_command(cmd, **kwargs, block=True, priority=11)
...
word_matcher = on_message(
    Rule(game_running) & get_word_input, block=True, priority=12)
