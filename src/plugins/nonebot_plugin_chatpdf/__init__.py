...
pdf_chat_request = on_command("chatpdf", block=True, priority=4)


@pdf_chat_request.handle()
async def _(event: GroupMessageEvent, msg: Message = CommandArg()):
    if event.get_session_id() not in data:
        await pdf_chat_request.finish(MessageSegment.text("请先使用pdfstart命令开始！"))
...
delete_request = on_command("delete_all", block=True, priority=4)
...
delete_user_request = on_command("delete_my", block=True, priority=4)
