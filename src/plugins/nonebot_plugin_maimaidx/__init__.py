...
async def _(event: Event, message: Message = CommandArg()):
    ...
    if success == 400:
        await best_40_pic.send("未找到此玩家，请确保此玩家的用户名和查分器中的用户名相同。绑定地址：https://www.diving-fish.com/maimaidx/prober/")
    elif success == 403:
...
async def _(event: Event, message: Message = CommandArg()):
    ...
    if success == 400:
        await best_50_pic.send("未找到此玩家，请确保此玩家的用户名和查分器中的用户名相同。绑定地址：https://www.diving-fish.com/maimaidx/prober/")
    elif success == 403:
