...
class Uploader(BaseModel):
    ...
    def dict(self, **kwargs) -> Dict[str, Any]:
        exclude_properties = {name for name, value in type(
            self).__dict__.items() if isinstance(value, property)}
        exclude_properties.add("living")
        exclude_properties.add("dyn_offset")
        if PYDANTIC_V2:
            dict_ = super().model_dump(
                **{**kwargs, "exclude": exclude_properties})  # type: ignore
        else:
            dict_ = super().dict(
                **{**kwargs, "exclude": exclude_properties})  # type: ignore
        return dict_
...
class UserSubConfig(BaseModel):
    ...
    def is_defualt_val(self) -> bool:
        ...
        if PYDANTIC_V2:
            return not any(
                (
                    self.__getattribute__(
                        field_name) != self.model_fields[field_name].default  # type: ignore
                    if field_name != "uid"
                    else False
                )
            ...
            return not any(
                (
                    self.__getattribute__(field_name) != self.__fields__[
                        field_name].default  # type: ignore
                    if field_name != "uid"
                    else False
                )
...
class User(BaseModel):
        ...
        for uid, sub in v.items():
            try:
                ...
                # 否则，显式地设置uid
                    ...
                    validated_subs[int(uid)] = UserSubConfig(
                        uid=int(uid), **sub)
    ...
    async def push_to_user(self, content: List[Union[str, bytes]], at_all: Optional[bool] = None):
        if not at_all:
            ...
            at = ""
        msg = UniMessage(at)
        msg.extend([Image(raw=x) if isinstance(x, bytes)
                   else x for x in content])  # type: ignore
    ...
    def add_subscription(self, uploader: Uploader) -> Union[None, str]:
        ...
        self.subscriptions[uploader.uid] = UserSubConfig(
            uid=uploader.uid)  # type: ignore
...
class SubscriptionConfig(BaseModel):
    subs_limit: int = Field(5, ge=0, le=500)
...
class SubscriptionSystem:
    ...
    async def load(cls, data: Dict[str, Union[Dict[str, Any], List[Dict[str, Any]]]]):
        ...
        async with CONFIG_LOCK:
            ...
            cls.users = {
                f"{u.platform}-_-{u.user_id}": u for u in raw_cfg.users}
    ...
    async def refresh_activate_uploaders(cls, refresh: bool = False):
        ...
        for user in cls.users.values():
            ...
            try:
                get_bots(target)
                cls.activate_uploaders.update(
                    {int(up): cls.uploaders[int(up)] for up in user.subscriptions.keys()})
            except NoBotFoundError:
