...
bili_add_sub = bilichat.command("sub", permission=GROUP_ADMIN | GROUP_OWNER |
                                SUPERUSER, aliases=set(plugin_config.bilichat_cmd_add_sub))
bili_remove_sub = bilichat.command("unsub", permission=GROUP_ADMIN | GROUP_OWNER |
                                   SUPERUSER, aliases=set(plugin_config.bilichat_cmd_remove_sub))
bili_check_sub = bilichat.command(
    "check", aliases=set(plugin_config.bilichat_cmd_check_sub))
bili_reset_sub = bilichat.command("reset", permission=GROUP_ADMIN | GROUP_OWNER | SUPERUSER, aliases=set(
    plugin_config.bilichat_cmd_reset_sub))
...
async def add_sub(uid: Message = CommandArg(), user: User = Depends(get_user), lock: Lock = Depends(check_lock)):
    async with lock:
        ...
        up = SubscriptionSystem.uploaders.get(
            msg.mid, Uploader(nickname=msg.nickname, uid=msg.mid))
    ...
    async with lock:
        ...
        # 查看指定 UP 主的配置
            ...
            for up in user.subscribe_ups:
                if up.nickname.lower() == keyword or str(up.uid) == keyword:
                    prompt = [str(up)]
                    cfg = user.subscriptions.get(
                        up.uid, UserSubConfig(uid=up.uid))
